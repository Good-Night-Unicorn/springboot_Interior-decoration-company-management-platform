	...
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="施工时间" prop="shigongshijian">
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.shigongshijian" 
						type="date"
						:readonly="ro.shigongshijian"
						placeholder="施工时间"
					></el-date-picker> 
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.shigongshijian" label="施工时间" prop="shigongshijian">
					<el-input v-model="ruleForm.shigongshijian" placeholder="施工时间" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="施工时长" prop="shigongshizhang">
					<el-input v-model="ruleForm.shigongshizhang" placeholder="施工时长" clearable  :readonly="ro.shigongshizhang"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="施工时长" prop="shigongshizhang">
					<el-input v-model="ruleForm.shigongshizhang" placeholder="施工时长" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="施工进度" prop="shigongjindu">
					<el-select :disabled="ro.shigongjindu" v-model="ruleForm.shigongjindu" placeholder="请选择施工进度" >
						<el-option
							v-for="(item,index) in shigongjinduOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="施工进度" prop="shigongjindu">
					<el-input v-model="ruleForm.shigongjindu"
						placeholder="施工进度" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="施工详情" prop="shigongxiangqing">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
					  placeholder="施工详情"
					  v-model="ruleForm.shigongxiangqing" >
					</el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.shigongxiangqing" label="施工详情" prop="shigongxiangqing">
					<span :style='{"fontSize":"14px","lineHeight":"40px","minWidth":"400px","color":"#000","fontWeight":"500","display":"inline-block"}'>{{ruleForm.shigongxiangqing}}</span>
				</el-form-item>
...
export default {
	data() {
		...
		return {
			...
			ro:{
				...
				shigongshijian : false,
				shigongshizhang : false,
				shigongxiangqing : false,
				kehuxinxi : false,
				shigongjindu : false,
	...
	created() {
		this.ruleForm.shigongshijian = this.getCurDate()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='shigongshijian'){
							this.ruleForm.shigongshijian = obj[o];
							this.ro.shigongshijian = true;
							continue;
						}
						if(o=='shigongshizhang'){
							this.ruleForm.shigongshizhang = obj[o];
							this.ro.shigongshizhang = true;
							continue;
						}
						if(o=='shigongxiangqing'){
							this.ruleForm.shigongxiangqing = obj[o];
							this.ro.shigongxiangqing = true;
							continue;
						}
						if(o=='kehuxinxi'){
						...
						if(o=='shigongjindu'){
							this.ruleForm.shigongjindu = obj[o];
							this.ro.shigongjindu = true;
							continue;
						}
				...
				this.ruleForm.shigongjindu = '施工中'; 			}
			// 获取用户信息
			this.$http({
            ...
            this.shigongjinduOptions = "施工中,已完成".split(',')
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `zhuangxiushigong/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.zhuangxiushigongCrossAddOrUpdateFlag = false;
											this.parent.search();
